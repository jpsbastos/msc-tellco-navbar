...
export class NavbarComponent extends React.Component<{}> {
      ...
      apiService
        ...
        .then((response: { data: { [key: string]: any } }) => {
          const event = new CustomEvent("signin", {
            detail: { userId: response.data._id },
          });
          dispatchEvent(event);
        });
    }
        ...
        <div className="tellco-navbar">
          <div className="navigation-container">
            <a href="/">
              <img
                className="logo"
                src="https://i.ibb.co/f9y8600/logo.png"
                alt="logo.png"
              />
            </a>
  ...
  private renderUserNameAndLogout(name: string): JSX.Element {
    return (
      <React.Fragment>
        ...
        <a className="login-btn" onClick={() => this.onLogout()}>
          Logout
        </a>
      </React.Fragment>
    );
  ...
  private renderLogin(): JSX.Element {
    return (
      <React.Fragment>
        <a className="login-btn" onClick={() => this.onSignIn()}>
          Sign-in
        </a>
        <a className="login-btn" onClick={() => this.onSignUp()}>
          Sign-up
        </a>
      </React.Fragment>
    );
  ...
  private handleOnChange(e: ChangeEvent<HTMLInputElement>, key: string): void {
    this.setState({
      modalForm: { ...this.state.modalForm, [key]: e.target.value },
    });
  }
  ...
  private renderSignout(): JSX.Element {
    return (
      <div className="modal-form">
        <h3>Sign-Out</h3>Do you want to sign-out?
      </div>
    );
  }
